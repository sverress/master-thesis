...
def system_simulate(world):
    ...
    flow_counter = {
        (start, end): 0
        for start in np.arange(len(world.state.clusters))
        for end in np.arange(len(world.state.clusters))
        if start != end
    }
    trips = []
    lost_demand = []
    scenario = random.choice(world.state.simulation_scenarios)
    for start_cluster_id, number_of_trips, end_cluster_indices in scenario:
        start_cluster = world.state.get_location_by_id(start_cluster_id)
        ...
        for j, end_cluster_index in enumerate(end_cluster_indices):
            trips.append(
                (
                    start_cluster,
                    world.state.get_location_by_id(end_cluster_index),
    ...
    for start_cluster, end_cluster, scooter in trips:
        ...
        trip_distance = world.state.get_distance(start_cluster.id, end_cluster.id)
